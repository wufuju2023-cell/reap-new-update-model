...
policy π_θ  : Qwen2.5-Math-7B 基座 + REAL-Prover 权重初始化
             ├─ LoRA 适配器（trainable; 4-bit? 不，用 BF16 全量 + LoRA rank16）
...
## 1.3 双 loss 训练（student 冷启动的两个信号）

### (a) 策略 SFT（离线，用 state_tactic_pairs）
$$\mathcal{L}_{\mathrm{pol}} = -\mathbb{E}_{(s,a)\sim D}\,\log \pi_\theta(a \mid s)$$

### (b) value head 监督（离线 + 在线混合）
初版：binary target = 该 (state,tactic) **是否 kernel 验证成功**：
$$\hat y = \mathbb{1}[\mathrm{checkProof}(s,a) = \mathrm{ok}],\quad \mathcal{L}_v = \mathrm{MSE}(V_\phi(s), \hat y)$$
进阶（在线）：MCTS 回报 $G_t$（discount 0.97）：
$$V_\phi(s_t) \leftarrow V_\phi(s_t) + \alpha_V\big(G_t - V_\phi(s_t)\big)$$

### (c) 训练执行化（P1 第一刀）

```
/opt/venv/bin/python  /workspace/app/train_sft.py --model FrenzyMath/REAL-Prover \
        --lora r=16 --epochs 2 --micro-bs 1 --gpus 4 --out /workspace/out/ckpt-sft
```
预期：6–8h（4 卡 * 50k pairs * 2 epochs）。**拆 4 批**（每批 12.5k），批间 checkpoints 可断。
...
- 二者与 Lean 侧（reapMCTS 的 endpoints 配置）对接时，策略地址填 localhost:8760（Lean 在实例内用同主机端口可行）
