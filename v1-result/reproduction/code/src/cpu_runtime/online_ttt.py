...
def run_online(*, session_id: str, project_dir: Path, theorem_file: str,
               ...
               max_updates: int = 5, http_timeout: float = DEFAULT_BUDGET.client,
    ...
    atomic_new_json(directory / "session.json", {**asdict(spec), **initialization, **pinned, **policy_fields,
    ...
    report = {"session_id": session_id, "tree_id": tree_id, "lean_pid": process.pid,
        ...
        "experience_id": experience_id, **policy_fields,
    ...
    if error is None:
        ...
        if experience_candidate and solved and coordinator.receipts:
