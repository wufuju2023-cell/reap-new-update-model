...
class OpenCliTransport:
    ...
    def _locked(self):
        ...
        if not self._thread_lock.acquire(timeout=60):
...
headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout(20000),body:JSON.stringify(__PAYLOAD__)});
return {status:r.status,text:await r.text()};
})()""".replace("__EXPECTED__", expected).replace("__PAYLOAD__", payload)
        ...
        try:
            with self._locked():
                ...
                completed = subprocess.run(
                    ...
                    shell=False, capture_output=True, text=True, encoding="utf-8", timeout=55,
                )
            stage = "cli_exit"
...
class Bridge:
    def __init__(self, transport, allowed_sessions: list[str], *, timeout_seconds: float = 600,
                 poll_interval: float = 1.0, clock=time.monotonic, sleep=time.sleep):
    ...
    def request(self, method: str, path: str, body: dict) -> tuple[int, bytes]:
        ...
        deadline = self.clock() + self.timeout_seconds + 60
        ...
        try:
            ...
            try:
                accepted = self.transport("submit", request)
            ...
            while self.clock() < deadline:
                try:
                    result = self.transport("poll", request_id=request_id, offset=len(chunks))
                except Exception as exc:
                    poll_failures += 1
...
def main() -> int:
    ...
    bridge = Bridge(transport, args.session_id, timeout_seconds=args.timeout_seconds)
