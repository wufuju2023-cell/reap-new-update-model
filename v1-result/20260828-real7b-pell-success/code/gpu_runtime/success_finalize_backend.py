...
def learn_success(backend, session_id, event):
    ...
    for row in example['samples']:
        prompt = backend.tokenizer(row['prompt'], return_tensors='pt').to(backend.device)
    ...
    try:
        with backend._session_rng(session):
            ...
            for row, prompt, target_ids in tokenized:
                ...
                target = torch.tensor(row['value_target'], device=backend.device, dtype=torch.float32)
                loss = backend.functional.mse_loss(prediction, target)
                ...
                trace.append({'row': row['row'], 'node_index': row['node_index'],
                    ...
                    'value_target': row['value_target'], 'target_tokens': length,
