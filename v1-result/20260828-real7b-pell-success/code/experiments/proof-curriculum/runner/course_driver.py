...
def prepare(*, source_root, store, family_id, course_record, budget_file, runtime_path,
    ...
    if premise_plan is not None:
        ...
        student_library_mode = premises.get('schema_version') == 'reap.teacher-course.student-proof-library.v1'
        require(premises.get('schema_version') in ('reap.teacher-course.premise-hints.v1', 'reap.teacher-course.student-proof-library.v1')
        ...
        require(isinstance(review, dict) and isinstance(review.get('reviewer'), str)
                and review['reviewer'].strip() and review.get('excludes_complete_goal') is True,
                'explicit premise semantic review required')
        if student_library_mode:
            require(review.get('verified_student_proofs_only') is True and review.get('basic_library_only') is False,
                    'explicit verified student-proof mode required; not a general-library claim')
        else:
            require(review.get('basic_library_only') is True, 'explicit basic-lemma semantic review required')
        ...
        for name in members:
            ...
            require(all((type(h) is dict and h.get('kind') == 'verified_student_proof') == student_library_mode for h in hints[name]),
                    'cannot mix student-proof and general-library premise modes')
            adapter.closed.prepare(environment_sha256=runtime['environment_sha256'],
                declarations=declarations.read_bytes(), closed_prop_name=name, polarity='prove',
