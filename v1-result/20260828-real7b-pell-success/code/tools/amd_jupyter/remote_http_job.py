...
MAX_BODY = 8192
MAX_BODY_OPT_IN = 256 * 1024
MAX_STAGED_REQUEST = MAX_BODY_OPT_IN + 64 * 1024
MAX_COMPRESSED_REQUEST = MAX_STAGED_REQUEST + 1024
UPLOAD_CHUNK = 12 * 1024
...
def body_limit(value=MAX_BODY) -> int:
    if type(value) is not int or not MAX_BODY <= value <= MAX_BODY_OPT_IN:
...
def validate_request(request: dict) -> str:
    ...
    limit = body_limit(request.get("max_request_body_bytes", MAX_BODY))
    if not isinstance(request.get("body"), dict) or len(json_bytes(request["body"])) > limit:
        raise ValueError(f"request body must be an object of at most {limit} bytes (default 8 KiB)")
...
def _new_staged_file(path: Path, data: bytes) -> None:
    # Partial writes remain evidence and cannot be silently replaced on retry.
...
def _read_staged_file(path: Path, limit: int) -> bytes:
    if path.is_symlink() or not path.is_file():
        raise ValueError("staged payload must be an ordinary file")
    with path.open("rb") as stream:
        raw = stream.read(limit + 1)
    if len(raw) > limit:
        raise ValueError("staged file exceeds bounded size")
    return raw


def _stage_metadata(metadata: dict) -> dict:
    fields = {"request_id", "request_sha256", "request_bytes", "compressed_sha256", "compressed_bytes", "encoding"}
    if not isinstance(metadata, dict) or set(metadata) != fields or metadata["encoding"] != "zlib":
        raise ValueError("invalid staged request metadata")
    if not re.fullmatch(r"[0-9a-f]{32}", str(metadata["request_id"])):
        raise ValueError("invalid staged request UUID")
    for key in ("request_sha256", "compressed_sha256"):
        if not isinstance(metadata[key], str) or re.fullmatch(r"[0-9a-f]{64}", metadata[key]) is None:
            raise ValueError("invalid staged request SHA256")
    for key, maximum in (("request_bytes", MAX_STAGED_REQUEST), ("compressed_bytes", MAX_COMPRESSED_REQUEST)):
        if type(metadata[key]) is not int or not 1 <= metadata[key] <= maximum:
            raise ValueError("staged request exceeds bounded size")
    return metadata


def stage_request(root: Path, metadata: dict, part_index: int, chunk: bytes) -> dict:
    """Store bytes only. No HTTP, worker, or training is launched here."""
    _stage_metadata(metadata)
    count = (metadata["compressed_bytes"] + UPLOAD_CHUNK - 1) // UPLOAD_CHUNK
    if type(part_index) is not int or not 0 <= part_index < count:
        raise ValueError("invalid staged request part")
    expected = min(UPLOAD_CHUNK, metadata["compressed_bytes"] - part_index * UPLOAD_CHUNK)
    if type(chunk) is not bytes or len(chunk) != expected:
        raise ValueError("staged request chunk length mismatch")
    target = job_dir(root / "_request_staging", metadata["request_id"])
    target.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
    try:
        target.mkdir(mode=0o700)
    except FileExistsError:
        if _read_staged_file(target / "metadata.json", 4096) != json_bytes(metadata):
            raise ValueError("staged request identity changed or preparation is incomplete")
    else:
        _new_staged_file(target / "metadata.json", json_bytes(metadata))
    path = target / f"part-{part_index:04d}.bin"
    try:
        _new_staged_file(path, chunk)
    except FileExistsError:
        if _read_staged_file(path, UPLOAD_CHUNK) != chunk:
            raise ValueError("staged request part differs; never replace")
    return {"state": "staged", "request_id": metadata["request_id"],
            "request_sha256": metadata["request_sha256"], "part_index": part_index,
            "chunk_sha256": hashlib.sha256(chunk).hexdigest()}


def submit_staged(root: Path, request_id: str, request_sha256: str, launcher=None) -> dict:
    """Validate the complete bounded stream before the sole job submission."""
    target = job_dir(root / "_request_staging", request_id)
    metadata = _stage_metadata(json.loads(_read_staged_file(target / "metadata.json", 4096)))
    if metadata["request_id"] != request_id or metadata["request_sha256"] != request_sha256:
        raise ValueError("staged submission pin mismatch")
    count = (metadata["compressed_bytes"] + UPLOAD_CHUNK - 1) // UPLOAD_CHUNK
    packed = bytearray()
    for index in range(count):
        part = _read_staged_file(target / f"part-{index:04d}.bin", UPLOAD_CHUNK)
        if len(part) != min(UPLOAD_CHUNK, metadata["compressed_bytes"] - len(packed)):
            raise ValueError("staged part length mismatch")
        packed.extend(part)
    if hashlib.sha256(packed).hexdigest() != metadata["compressed_sha256"]:
        raise ValueError("staged compressed checksum mismatch")
    decoder = zlib.decompressobj()
    raw = decoder.decompress(packed, metadata["request_bytes"] + 1)
    if (len(raw) != metadata["request_bytes"] or not decoder.eof or decoder.unused_data
            or decoder.unconsumed_tail or hashlib.sha256(raw).hexdigest() != request_sha256):
        raise ValueError("staged request length/checksum/EOF mismatch")
    request = json.loads(raw)
    if json_bytes(request) != raw or request.get("request_id") != request_id:
        raise ValueError("staged request must be canonical and match its UUID")
    if body_limit(request.get("max_request_body_bytes", MAX_BODY)) == MAX_BODY:
        raise ValueError("staged transport requires an explicit larger request limit")
    validate_request(request)
    return submit_job(root, request, launcher=launcher)
...
def main() -> int:
    ...
    parser.add_argument("action", choices=("submit", "poll", "run", "stage", "submit-staged"))
    ...
    parser.add_argument("--chunk-base64")
    ...
    if args.action == "submit":
        ...
        result = stage_request(args.root, metadata, args.part_index, base64.b64decode(args.chunk_base64, validate=True))
    elif args.action == "submit-staged":
        result = submit_staged(args.root, args.request_id, args.request_sha256)
