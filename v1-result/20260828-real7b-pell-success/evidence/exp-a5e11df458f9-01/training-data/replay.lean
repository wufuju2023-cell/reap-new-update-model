...
/-- Advance the lower successor coordinate beyond the next bound. -/
def CourseLowerCoordinateGrowth : Prop :=
  ∀ (B x y : ℕ), B < x → B < y → B + 1 < 2 * x + 3 * y
...
run_cmd do
  ...
  let name : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CourseLowerCoordinateGrowth"
...
theorem ReapCurriculumWrapper.attempt : _root_.CodexMathFive.Pell.CourseLowerCoordinateGrowth := by
  unfold _root_.CodexMathFive.Pell.CourseLowerCoordinateGrowth
  reapVerifiedReplay "${HISTORICAL_RUNS}/radeon-pell-lower-coordinate-code04-20260828-native/replay-000/plan.json" "${HISTORICAL_RUNS}/radeon-pell-lower-coordinate-code04-20260828-native/replay-000/trace.json"

open Lean in
run_cmd do
  let target : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CourseLowerCoordinateGrowth"
