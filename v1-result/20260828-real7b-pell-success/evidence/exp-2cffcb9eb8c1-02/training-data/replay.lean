...
run_cmd do
  ...
  let name : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CoursePositiveGrowth"
...
def ReapCurriculumWrapper.expandDefinitions (original : Lean.Expr) : Lean.MetaM Lean.Expr := do
  let target : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CoursePositiveGrowth"
...
theorem ReapCurriculumWrapper.attempt : _root_.CodexMathFive.Pell.CoursePositiveGrowth := by
  run_tac
  ...
  reapVerifiedReplay "${HISTORICAL_RUNS}/radeon-pell-success-code04-20260828-native/pell-replay-001/plan.json" "${HISTORICAL_RUNS}/radeon-pell-success-code04-20260828-native/pell-replay-001/trace.json"
