...
/-- Construct witnesses with specified coordinates, then prove the full successor property. -/
def CourseWitnessSpecifiedSuccessor : Prop :=
  ∀ (B x y : ℕ), B < x → B < y → x ^ 2 = 2 * y ^ 2 + 1 →
    ∃ u v : ℕ, u = 3 * x + 4 * y ∧ v = 2 * x + 3 * y ∧
...
run_cmd do
  ...
  let name : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CourseWitnessSpecifiedSuccessor"
...
theorem ReapCurriculumWrapper.attempt : _root_.CodexMathFive.Pell.CourseWitnessSpecifiedSuccessor := by
  unfold _root_.CodexMathFive.Pell.CourseWitnessSpecifiedSuccessor
  reapVerifiedReplay "${HISTORICAL_RUNS}/radeon-pell-witness-successor-code11-20260828-native/replay-000/plan.json" "${HISTORICAL_RUNS}/radeon-pell-witness-successor-code11-20260828-native/replay-000/trace.json"

open Lean in
run_cmd do
  let target : Lean.Name := Lean.Name.str (Lean.Name.str (Lean.Name.str (Lean.Name.anonymous) "CodexMathFive") "Pell") "CourseWitnessSpecifiedSuccessor"
